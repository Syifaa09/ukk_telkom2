...
class AlbumController extends Controller
{
    ...
    {
        $request->validate(['nama_album' => 'required|string|max:255', 'deskripsi' => 'required|string', ]);
        $album = new Album(['nama_album' => $request->nama_album, 'deskripsi' => $request->deskripsi, 'user_id' => auth()->id() , // Otomatis gunakan ID user yang sedang login
        ]);
        $album->save();
    ...
    public function update(Request $request, $id)
    {
        $request->validate(['nama_album' => 'required|string|max:255', 'deskripsi' => 'required|string', ]);
        $album = Album::findOrFail($id);
        $album->update(['nama_album' => $request->nama_album, 'deskripsi' => $request->deskripsi, ]);
